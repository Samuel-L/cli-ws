...
def _create_task_file(name):
	...
	with open(f'{documents_folder_path}/taskfile.csv', 'w', newline='') as taskfile:
...
def create_task(task_name, url, target,
	...
	"""
	documents_folder_path = os.path.expanduser('~/Documents/cli_ws_1.0.0')
	if not os.path.isfile(f'{documents_folder_path}/taskfile.csv'):
		_create_task_file('taskfile.csv')

	with open(f'{documents_folder_path}/taskfile.csv', 'a', newline='') as taskfile:
		writer = csv.writer(taskfile, delimiter=',', quotechar='"',
...
def return_task(task_name='', task_group='', all_tasks=False):
	...
	"""
	documents_folder_path = os.path.expanduser('~/Documents/cli_ws_1.0.0')
	with open(f'{documents_folder_path}/taskfile.csv', 'r') as taskfile:
...
def remove_task(task_name):
	...
	documents_folder_path = os.path.expanduser('~/Documents/cli_ws_1.0.0')
	with open(f'{documents_folder_path}/taskfile.csv', 'r') as taskfile, open('output.csv', 'w') as outfile:
	...
	shutil.move('output.csv',f'{documents_folder_path}/taskfile.csv')
